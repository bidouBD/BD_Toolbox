...
### 3. 🛠️ 进阶处理工具
*   **字幕大师**：支持硬压字幕到视频，或从视频中提取字幕流。
*   **音频专家**：支持音频一键提取、AAC/MP3 转换。
*   **视频合并**：基于列表的高速视频合并。
...
```bash
# 克隆仓库
git clone https://github.com/bidouBD/BD_Toolbox.git
...
### 打包命令
```bash
pyinstaller --noconfirm --onedir --windowed --add-data "bin;bin" main.py
```
